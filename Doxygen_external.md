...
To generate a tag file for your project, simply put the name of the tag file after the GENERATE\_TAGFILE  option in the configuration file.
...
After compile time:
> if you do not assign a location to a tag file, doxygen will generate dummy links for all external HTML references. It will also generate a perl script called [installdox](Doxygen_installdox_usage.md)  in the HTML output directory. This script should be run to replace the dummy links with real links for all generated HTML files.
...
In some (hopefully exceptional) cases you may have the documentation generated by doxygen, but not the sources nor a tag file. In this case you can use the [doxytag](Doxygen_doxytag_usage.md)  tool to extract a tag file from the generated HTML sources. Another case where you should use doxytag is if you want to create a tag file for the Qt documentation.
