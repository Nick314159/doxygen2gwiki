...
> 2. Create a copy of translator\_en.h and name it translator

<your\_2\_letter\_country\_code>

.h I'll use xx in the rest of this document.
> 3. Add definition of the symbol for your language into lang\_cfg.h:
```
#define LANG_xx
```
...
#endif
```
Remember to use the same symbol LANG\_xx that you added to `lang_cfg.h`. I.e., the `xx` should be capital letters that identify your language. On the other hand, the `xx` inside your `translator_xx.h` should use lower case.
...
> 6. Edit `translator_xx.h`:
  ...
  * Enter them directly if your keyboard supports that and you are using a Latin-1 font. Doxygen will translate the characters to proper ![http://doxygen2gwiki.googlecode.com/svn/wiki/Doxygen_form_0.png](http://doxygen2gwiki.googlecode.com/svn/wiki/Doxygen_form_0.png)  and leave the HTML and man output for what it is (which is fine, if idLanguageCharset() is set correctly).
